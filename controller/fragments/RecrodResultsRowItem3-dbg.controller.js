...
sap.ui.define([
	...
	return RecrodResultsRowItemBaseController.extend("i2d.qm.inspresults.records1.controller.fragments.RecrodResultsRowItem3", {
		formatter: formatter,
		...
		onSingleValueMultiInputTokenChange: function(oEvent) {
			...
			var iUpperNumber = 0;
			var iLowerNumber = 0;
			inputValues.forEach(function(iValue) {
				if (oBindingData.IsUpper && iValue > oBindingData.Inspspecupperlimit) {
					iUpperNumber++;
				}
				if (oBindingData.IsLower && iValue < oBindingData.Inspspeclowerlimit) {
					iLowerNumber++;
				}
			});

			oBindingData.InspRsltAboveToleranceValsNmbr = iUpperNumber;
			oBindingData.InspRsltBelowToleranceValsNmbr = iLowerNumber;
			oBindingData.InspRsltNonconformingValsNmbr = iUpperNumber + iLowerNumber;
